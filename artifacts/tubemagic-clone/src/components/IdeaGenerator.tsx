...
export default function IdeaGenerator() {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
      <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Video Idea Generator</h2>
      <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.45)', marginBottom: '24px' }}>Get 8 high-potential ideas for your niche</p>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        ...
        <button
          ...
          style={{ padding: '12px 20px', borderRadius: '10px', background: '#fff', color: '#000', fontWeight: 600, fontSize: '14px', cursor: niche.trim() && !loading ? 'pointer' : 'not-allowed', opacity: (!niche.trim() || loading) ? 0.5 : 1, border: 'none', whiteSpace: 'nowrap' }}
        >
          ...
          {ideas.map((idea, i) => (
            <div key={i} style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '18px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px', marginBottom: '8px' }}>
                <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#fff', lineHeight: 1.4, flex: 1 }}>{idea.title}</h3>
              ...
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.6 }}>{idea.description}</p>
            </div>
          ))}
        </div>
      )}
