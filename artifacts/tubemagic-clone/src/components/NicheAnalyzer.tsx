...
export default function NicheAnalyzer() {
  ...
  return (
    <div style={{ maxWidth: '980px', margin: '0 auto', padding: '40px 24px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Niche Analyzer</h1>
      <p style={{ fontSize: '17px', color: 'rgba(255,255,255,0.45)', marginBottom: '24px' }}>Discover profitable niches or analyze any niche using live YouTube data</p>
      ...
      <div style={{ display: 'flex', gap: '4px', background: '#111', borderRadius: '10px', padding: '4px', marginBottom: '24px', width: 'fit-content' }}>
        {(['discover', 'analyze'] as const).map(m => (
          <button key={m} onClick={() => setMode(m)} style={{
            padding: '7px 18px', borderRadius: '7px', fontSize: '17px', fontWeight: 500, border: 'none', cursor: 'pointer',
        ...
        <div>
          {discovered.length === 0 && !discovering && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px', gap: '20px', textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: '24px', fontWeight: 700, color: '#fff' }}>Find Your Profitable Niche</h2>
              <p style={{ fontSize: '24px', color: 'rgba(255,255,255,0.5)', maxWidth: '480px', lineHeight: 1.6 }}>
              ...
              <button onClick={discover} style={{ padding: '14px 32px', borderRadius: '10px', background: '#fff', color: '#000', fontWeight: 700, fontSize: '17px', border: 'none', cursor: 'pointer' }}>
          ...
          {discovering && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 24px', gap: '16px', color: 'rgba(255,255,255,0.5)', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '24px' }}>Scanning YouTube data for profitable niches...</p>
          ...
          {discovered.length > 0 && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <span style={{ fontSize: '17px', color: 'rgba(255,255,255,0.4)' }}>{discovered.length} niches found — click any to get a full YouTube-powered analysis</span>
                <button onClick={discover} style={{ padding: '6px 14px', borderRadius: '7px', background: 'rgba(255,255,255,0.07)', color: 'rgba(255,255,255,0.6)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '24px', cursor: 'pointer' }}>↻ Refresh</button>
              </div>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '24px', fontWeight: 600, color: '#fff', lineHeight: 1.3, marginBottom: '4px' }}>{n.niche}</div>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <div style={{ fontSize: '24px', fontWeight: 700, color: '#f59e0b' }}>{n.cpmRange}</div>
                        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>CPM</div>
                      </div>
                    </div>

                    <p style={{ fontSize: '24px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.6, margin: 0 }}>{n.why}</p>
                    ...
                    {n.avgTopViews !== undefined && n.avgTopViews > 0 && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.04)', borderRadius: '6px', padding: '6px 10px' }}>
                        <span style={{ fontSize: '17px', color: 'rgba(255,255,255,0.3)' }}>Avg top video:</span>
                        <span style={{ fontSize: '24px', fontWeight: 600, color: '#fff' }}>{formatViews(n.avgTopViews)} views</span>
                      </div>
                    )}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'rgba(255,255,255,0.3)', fontSize: '24px', marginTop: 'auto', paddingTop: '4px' }}>
      ...
      {mode === 'analyze' && (
        <div>
          ...
          <textarea value={channelDesc} onChange={e => setChannelDesc(e.target.value)} placeholder="Optional: describe your channel or content style..."
            rows={2} style={{ width: '100%', background: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '12px 16px', color: '#fff', fontSize: '17px', fontFamily: 'inherit', outline: 'none', resize: 'none', marginBottom: '16px', boxSizing: 'border-box' }} />
          ...
          {loading && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px', gap: '12px', color: 'rgba(255,255,255,0.4)', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '24px' }}>Pulling live YouTube data + running AI analysis...</p>
            </div>
          )}
          ...
          {analysis && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                {[
                  ...
                  <div key={i} style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '14px 16px', textAlign: 'center' }}>
                    <div style={{ fontSize: '17px', color: 'rgba(255,255,255,0.4)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</div>
                    <div style={{ fontSize: '24px', fontWeight: 700, color: (s.colors as Record<string,string>)[s.value] || '#fff' }}>{s.value}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '20px 24px' }}>
                <div style={{ fontSize: '17px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '10px' }}>AI Analysis</div>
                <p style={{ fontSize: '24px', color: 'rgba(255,255,255,0.8)', lineHeight: 1.7, margin: 0 }}>{analysis.summary}</p>
              </div>
              ...
              {topVideos.length > 0 && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                    <span style={{ fontSize: '17px', fontWeight: 600, color: '#ef4444' }}>▶ Live YouTube Data</span>
                    <span style={{ fontSize: '17px', color: 'rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: '20px' }}>Top {topVideos.length} videos by views</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {topVideos.slice(0, 8).map((v, i) => (
                      <a key={v.videoId} href={`https://youtube.com/watch?v=${v.videoId}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                        ...
                        >
                          <span style={{ fontSize: '17px', color: 'rgba(255,255,255,0.2)', fontWeight: 600, minWidth: '20px', textAlign: 'right' }}>#{i + 1}</span>
                          <img src={v.thumbnail} alt="" style={{ width: '72px', height: '40px', objectFit: 'cover', borderRadius: '4px', flexShrink: 0 }} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: '17px', fontWeight: 500, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '3px' }}>{v.title}</div>
                            <div style={{ fontSize: '17px', color: 'rgba(255,255,255,0.4)' }}>{v.channel} · {timeAgo(v.publishedAt)}</div>
                          </div>
                          <div style={{ textAlign: 'right', flexShrink: 0 }}>
              ...
              {analysis.titlePatterns && analysis.titlePatterns.length > 0 && (
                <div>
                  <div style={{ fontSize: '17px', fontWeight: 600, color: '#a78bfa', marginBottom: '10px' }}>📝 Title Patterns That Work</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {analysis.titlePatterns.map((p, i) => (
                      <div key={i} style={{ background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: '8px', padding: '8px 14px', fontSize: '17px', color: 'rgba(255,255,255,0.8)' }}>"{p}"</div>
                    ))}
                  </div>
                </div>
              )}
              ...
              {/* Opportunities */}
              <div>
                <div style={{ fontSize: '17px', fontWeight: 600, color: '#f59e0b', marginBottom: '12px' }}>🚀 Content Opportunities</div>
                  ...
                  {analysis.opportunities.map((o, i) => (
                    <div key={i} style={{ background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                        <div style={{ fontSize: '24px', fontWeight: 600, color: '#fff' }}>{o.title}</div>
                      ...
                      <p style={{ fontSize: '24px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.6, margin: 0 }}>{o.description}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Content Gaps */}
              <div>
                <div style={{ fontSize: '17px', fontWeight: 600, color: '#3b82f6', marginBottom: '12px' }}>🔍 Untapped Gaps</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {analysis.contentGaps.map((g, i) => (
                    <div key={i} style={{ background: '#111', border: '1px solid rgba(59,130,246,0.15)', borderRadius: '10px', padding: '14px 16px', display: 'flex', gap: '12px' }}>
                      ...
                      <div>
                        <div style={{ fontSize: '24px', fontWeight: 600, color: '#fff', marginBottom: '4px' }}>{g.gap}</div>
                        <div style={{ fontSize: '24px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.5 }}>{g.why}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Formats */}
              <div>
                <div style={{ fontSize: '17px', fontWeight: 600, color: '#22c55e', marginBottom: '12px' }}>🎬 Best Performing Formats</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px' }}>
                  {analysis.topFormats.map((f, i) => (
                    <div key={i} style={{ background: '#111', border: '1px solid rgba(34,197,94,0.1)', borderRadius: '12px', padding: '16px' }}>
                      <div style={{ fontSize: '24px', fontWeight: 600, color: '#fff', marginBottom: '6px' }}>{f.format}</div>
                      <div style={{ fontSize: '24px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.5, marginBottom: '8px' }}>{f.description}</div>
                      <div style={{ fontSize: '17px', color: 'rgba(34,197,94,0.7)', fontStyle: 'italic' }}>e.g. {f.examples}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '20px' }}>
                  <div style={{ fontSize: '17px', fontWeight: 600, color: '#22c55e', marginBottom: '14px' }}>⚡ Quick Wins</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {analysis.quickWins.map((w, i) => (
                      <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                        <span style={{ color: '#22c55e', flexShrink: 0, marginTop: '1px' }}>✓</span>
                        <span style={{ fontSize: '17px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>{w}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '20px' }}>
                  <div style={{ fontSize: '17px', fontWeight: 600, color: '#ef4444', marginBottom: '14px' }}>⚠️ Avoid These Mistakes</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {analysis.avoidMistakes.map((m, i) => (
                      <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                        <span style={{ color: '#ef4444', flexShrink: 0, marginTop: '1px' }}>✗</span>
                        <span style={{ fontSize: '17px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>{m}</span>
                      </div>
                    ))}
                  </div>
                </div>
