...
export default function ThumbnailGenerator() {
  ...
  return (
    <div style={{ maxWidth: '780px', margin: '0 auto', padding: '40px 24px' }}>
      <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Thumbnail Text Generator</h1>
      <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.45)', marginBottom: '28px' }}>Get high-CTR text overlay ideas for your thumbnail — the text that makes people stop scrolling</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
        ...
        <button onClick={generate} disabled={!title.trim() || loading} style={{
          padding: '12px', borderRadius: '10px', fontWeight: 600, fontSize: '15px', border: 'none',
          ...
          {suggestions.map((s, i) => (
            <div key={i} style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', overflow: 'hidden' }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 'clamp(20px, 3vw, 28px)', fontWeight: 800, color: '#fff', textAlign: 'center', lineHeight: 1.2, letterSpacing: '-0.5px', textTransform: 'uppercase', textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>
                  {s.mainText}
                </div>
                {s.subText && (
                  <div style={{ fontSize: 'clamp(12px, 1.8vw, 16px)', fontWeight: 600, color: 'rgba(255,255,255,0.75)', textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    {s.subText}
                  </div>
                )}
              ...
              <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <div>
                  <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>{s.style}</div>
                  <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.5 }}>{s.why}</div>
                </div>
                <button onClick={() => copy(s, i)} style={{ flexShrink: 0, padding: '6px 14px', borderRadius: '6px', fontSize: '12px', background: copiedIdx === i ? '#22c55e20' : 'rgba(255,255,255,0.08)', color: copiedIdx === i ? '#22c55e' : 'rgba(255,255,255,0.5)', border: `1px solid ${copiedIdx === i ? '#22c55e40' : 'rgba(255,255,255,0.1)'}`, cursor: 'pointer' }}>
                  {copiedIdx === i ? '✓' : 'Copy'}
                </button>
              </div>
            </div>
